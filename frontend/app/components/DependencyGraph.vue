...
<script lang="ts" setup>
  ...
  const graphData = computed(() => {
    ...
    for (const nodeId of dg.nodes() as string[]) {
      ...
      if (dgNode) layoutNodes[nodeId] = { x: dgNode.x, y: dgNode.y }
    }
  ...
  const graphHeight = computed(() => {
    ...
    if (ys.length === 0) return props.height
  ...
  const eventHandlers = {
    ...
    'node:pointerover': ({ node, event }: { node: string, event: PointerEvent }) => {
      ...
      if (svg) svg.style.cursor = data?.known ? 'pointer' : 'default'
    },
  }

  watch(graphData, v => {
    if (v && networkGraph.value) networkGraph.value.fitToContents()
  })
</script>
