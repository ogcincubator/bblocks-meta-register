...
async def _crawl_one_register(client, semaphore: asyncio.Semaphore, register_info: RegisterInfo) -> None:
    async with semaphore:
        ...
        try:
            ...
            chunks, embeddings, accepted_bblocks, descriptions, chunking_failed_ids, bindings = await build_search_content(
            ...
            async with session_scope() as session:
                await write_search_content(
                    session, register_info, chunks, embeddings, accepted_bblocks, descriptions, bindings
                )
